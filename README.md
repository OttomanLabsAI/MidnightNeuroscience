...
Brain mapping in the browser. Six instruments, one site, no server:
...
- **Networks** (`site/networks.html`) — brain states drawn as a circular graph: fourteen regions in seven networks, with a thread between every pair whose coupling changes in the state chosen. Six states — flow, rest, focused attention, threat, reward pursuit and meditation — each with its own sources. Threads that strengthen are drawn in the two regions' colours; those that fall quiet are grey and dashed.
- **Network Atlas** (`site/network-atlas.html`) — the same ring beside the scan. Click a region on the ring and it lights up on the brain in its own colour, with the planes centring on it and the regions it couples to held at half strength. Where a region has no atlas parcel — the ventral striatum, the VTA, the locus coeruleus — the page says so and parks the crosshair at a published coordinate rather than borrowing a neighbour.
