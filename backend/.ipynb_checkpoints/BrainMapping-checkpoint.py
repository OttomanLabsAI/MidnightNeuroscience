...
def ReceptorFilter(df, receptor):
    """
    ...
    """
    # Ensure receptor keyword is lowercase for case-insensitive matching
    receptor = receptor.lower()
    
    df_new = df.copy()

    # Identify columns that contain the receptor keyword (case-insensitive)
    filtered_columns = [col for col in df.columns if receptor in col.lower()]

    # Check for a 'Brain structure' column (case-insensitive)
    brain_structure_column = next((col for col in df.columns if col.lower() == 'brain structure'), None)

    if brain_structure_column:
        # Add 'Brain structure' (renamed to 'Brain Structure') to the filtered columns if it's not already included
        filtered_df = df_new[[brain_structure_column] + filtered_columns]
        filtered_df = filtered_df.rename(columns={brain_structure_column: 'Brain Structure'})
        
        # Set 'Brain Structure' as the index
        filtered_df = filtered_df.set_index('Brain Structure')
    else:
        # If 'Brain structure' is not found, just filter by the receptor columns
        filtered_df = df_new[filtered_columns]
